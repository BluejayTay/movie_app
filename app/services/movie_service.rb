class MovieService
  ...
  def self.popular
    response = HTTParty.get("https://api.themoviedb.org/3/discover/movie?api_key=ab2fd25c159a295d784ed92cec59a73b&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1");
    response["results"];
  end
  def self.title_search(query)
    response = HTTParty.get("https://api.themoviedb.org/3/search/movie?api_key=ab2fd25c159a295d784ed92cec59a73b&language=en-US&query=" + "#{query}" + "&page=1&include_adult=false");
    response["results"];
  end
  def self.find_by_api_id(api_id)
    response = HTTParty.get("https://api.themoviedb.org/3/movie/" + "#{api_id}" + "?api_key=ab2fd25c159a295d784ed92cec59a73b&language=en-US");
  end
end